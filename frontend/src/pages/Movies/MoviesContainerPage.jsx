...
import {useGetTopMoviesQuery, useGetRandomMovieQuery,useGetNewMoviesQuery} from '../../App/api/moviesApi';
...
const MoviesContainerPage = () => {
    ...
    const {data: randomMovies, error, isLoading, isError} = useGetRandomMovieQuery();
  ...
  return (
    <div className='flex flex-col lg:flex-row lg:justify-between items-center'>
        <nav className='ml-[4rem] flex flex-row xl:flex-col lg:flex-col md:flex-row sm:flex-row'>
            {genres?.map(g =>(
                <button key={g._id} className={`transition duration-300 ease-in-out hover:bg-gray-200 p-2 rounded mb-[1rem] text-lg ${selectedGenre === g._id ? "bg-gray-200" : ""}`}
